...
init_database() {
    ...
    # 移除注释后提取值（使用更精确的正则表达式）
    DB_HOST=$(grep -E "\$_ENV\['db_host'\]" config/.config.php 2>/dev/null | head -1 | sed 's|//.*||' | sed "s/.*= ['\"]\([^'\"]*\)['\"].*/\1/" | tr -d " \t")
    ...
    if [ -z "$DB_NAME" ] || [ -z "$DB_USER" ] || [ -z "$DB_PASS" ]; then
        DB_HOST=$(grep -E "\$_ENV\['db_host'\]" config/.config.php 2>/dev/null | head -1 | sed "s/.*= ['\"]\([^'\"]*\)['\"].*/\1/" | tr -d " \t")
        DB_NAME=$(grep -E "\$_ENV\['db_database'\]" config/.config.php 2>/dev/null | head -1 | sed "s/.*= ['\"]\([^'\"]*\)['\"].*/\1/" | tr -d " \t")
        DB_USER=$(grep -E "\$_ENV\['db_username'\]" config/.config.php 2>/dev/null | head -1 | sed "s/.*= ['\"]\([^'\"]*\)['\"].*/\1/" | tr -d " \t")
        DB_PASS=$(grep -E "\$_ENV\['db_password'\]" config/.config.php 2>/dev/null | head -1 | sed "s/.*= ['\"]\([^'\"]*\)['\"].*/\1/" | tr -d " \t")
