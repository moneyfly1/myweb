            ...
            <div class="import-section">
              ...
              <div class="sub-replace-row">
                ...
                <span class="subscription-tip">
                  {{ subReplaceMode ? '增量更新该订阅下的节点：匹配到的节点更新配置并保留用户分配，订阅中已消失的旧节点保留不删（适用于更换订阅地址或内容更新，分配保护）' : '仅追加新节点：同一个节点（地址+凭据/传输参数全同）自动跳过；同一 IP:端口但配置不同的节点会正常导入' }}
                </span>
              </div>
            ...
            <div class="import-section">
              ...
              <el-form :model="vpsForm" label-position="top" class="vps-form">
                ...
                <el-form-item label="VPS IP / 域名" required>
                  <el-input v-model="vpsForm.ssh_host" placeholder="如: 203.0.113.38 或 vps.example.com" />
                </el-form-item>
