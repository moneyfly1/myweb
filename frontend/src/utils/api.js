...
export const orderAPI = {
  ...
  getOrderDetail: (id) => api.get(`/orders/id/${id}`), // 订单详情（与列表同形状：formatOrderData）
...
export const adminAPI = {
  ...
  markUserNormal: (id) => api.post(`/admin/users/abnormal/${id}/mark-normal`),
  getUserDetails: (id, params) => api.get(`/admin/users/${id}/details`, { params }),
  ...
  // 节点管理方法与 nodeAPI 一致，直接复用避免双份维护
  getAdminNodes: (params) => nodeAPI.getAdminNodes(params),
  createNode: (data) => nodeAPI.createNode(data),
  ...
  getNodeLink: (id) => nodeAPI.getNodeLink(id),
  ...
  // 更新订阅（mode=update）：replace=true 全范围匹配更新（更换订阅地址场景，匹配同名/同地址节点并保留分配）；
  // replace=false 仅更新该订阅 URL 下的节点（同一订阅内容更新）。
  updateCustomNodeSubscription: (url, replace = true) => api.post('/admin/custom-nodes/import-subscription', { url, replace, mode: 'update' }, { timeout: 200000, _retry: true }),
  ...
  updateUserCustomNodeQuota: (nodeId, userId, data) => api.put(`/admin/custom-nodes/user-quota/${nodeId}/${userId}`, data),
