...
// hy2:// 是 Hysteria2 的通用简写 scheme，大量订阅源（如 Zefly）使用它。
// 此前采集链路只识别 hysteria2://，导致 hy2:// 节点被静默丢弃。

const testHy2Link = "hy2://b07db61f-a207-4508-97f5-68182991c2d6@152.69.220.212:35000?sni=api.push.apple.com&insecure=0#Zefly-%E6%96%B0%E5%8A%A0%E5%9D%A1-vip-1772"
...
func TestParseNodeLink_Hy2Scheme(t *testing.T) {
	...
	if node.Server != "152.69.220.212" {
		t.Errorf("服务器地址错误: %q", node.Server)
	}
	if node.Port != 35000 {
		t.Errorf("端口错误: %d", node.Port)
	}
	if node.Password != "b07db61f-a207-4508-97f5-68182991c2d6" {
	...
	if node.Name != "Zefly-新加坡-vip-1772" {
...
func TestExtractNodeLinks_Hy2Base64Subscription(t *testing.T) {
	...
	plain := strings.Join([]string{
		testHy2Link,
		"hy2://pwd@203.27.106.148:60000?sni=www.apple.com&insecure=0#Zefly-SG-vip-1570",
		"vless://uuid-1234@203.10.96.138:443?security=reality&sni=cdn.com&pbk=KEY&sid=abc#Vless-US",
	}, "\n")
