...
func SetupRouter() *gin.Engine {
	...
	api := r.Group("/api/v1")
	{
		...
		// 自建节点 agent 端点（无鉴权，凭 install_id+token，须在 CSRF 中间件之前）
		api.GET("/agent/install.sh", handlers.AgentInstallScript)
		api.POST("/agent/report", handlers.AgentReport)
		api.POST("/agent/report-batch", handlers.AgentReportBatch)
		api.POST("/agent/heartbeat", handlers.AgentHeartbeat)
		...
		admin.Use(middleware.NoStoreMiddleware())
		{
			...
			admin.POST("/nodes/selfhost", handlers.CreateSelfHostNode)
			admin.GET("/nodes/selfhost", handlers.GetSelfHostNodes)
			admin.GET("/nodes/selfhost/:id", handlers.GetSelfHostNodeStatus)
			admin.POST("/nodes/disable-timeout", handlers.DisableTimeoutNodes)
			admin.POST("/nodes/enable-all", handlers.EnableAllNodes)

			admin.GET("/custom-nodes", handlers.GetCustomNodes)
			admin.GET("/custom-nodes/:id/users", handlers.GetCustomNodeUsers)
			admin.PUT("/custom-nodes/user-quota/:nodeId/:userId", handlers.UpdateUserCustomNodeQuota)
			...
			admin.POST("/custom-nodes/selfhost/:id/reset-traffic", handlers.SelfHostResetTraffic)
			admin.POST("/custom-nodes/selfhost/:id/update-ssh", handlers.SelfHostUpdateSSH)
			admin.GET("/custom-nodes/selfhost", handlers.GetSelfHostNodes)
			admin.GET("/custom-nodes/selfhost/:id", handlers.GetSelfHostNodeStatus)
